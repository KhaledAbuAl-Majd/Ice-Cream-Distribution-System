...
{
    public class frmRepresentatives : Form
    {
        ...
        private Representative? _selected;

        public frmRepresentatives() { BuildUI(); WireEvents(); _ = LoadAsync(); }

        private void BuildUI()
        {
            BackColor = AppColors.PrimaryDark; RightToLeft = RightToLeft.Yes; RightToLeftLayout = true;
            Controls.Add(new Label { Text = "إدارة المناديب", ForeColor = AppColors.TextPrimary, Font = new Font("Segoe UI", 14f, FontStyle.Bold), AutoSize = true, Location = new Point(20, 20) });
        ...
        private void WireEvents()
        {
            ...
            {
                if (dgv.CurrentRow?.DataBoundItem is not Representative r) return;
                _selected = r;
                foreach (var item in cmbUser.Items) { if ((item as dynamic)?.UserId == r.UserId) { cmbUser.SelectedItem = item; break; } }
                foreach (var item in cmbCar.Items) { if ((item as dynamic)?.Id == r.CarId) { cmbCar.SelectedItem = item; break; } }
            };
        }
        ...
        private async Task LoadAsync()
        {
            try
            {
                ...
                dgv.DataSource = await RepresentativeService.GetAll() ?? new();
                ...
                dgv.Columns[nameof(Representative.Id)].Visible = true;
                dgv.Columns[nameof(Representative.CarId)].Visible = true;
                dgv.Columns[nameof(Representative.UserId)].Visible = true;
            }
        ...
        private async void BtnSave_Click(object? s, EventArgs e)
        {
            ...
            try
            {
                if (_selected is null) await RepresentativeService.Add(new Representative { UserId = userId, CarId = carId });
                else { _selected.UserId = userId; _selected.CarId = carId; await RepresentativeService.Update(_selected); }
        ...
        {
            if (_selected is null) { clsPL_MessageBoxs.ShowErrorMessage("اختر مندوباً أولاً"); return; }
            if (clsPL_MessageBoxs.ShowConfirmMessage("حذف المندوب؟") != DialogResult.Yes) return;
            try { await RepresentativeService.Delete(_selected.Id); clsPL_MessageBoxs.ShowSuccessMessage("تم الحذف"); ClearForm(); await LoadAsync(); }
            catch (Exception ex) { clsPL_MessageBoxs.ShowErrorMessage(ex.Message); }
        }
